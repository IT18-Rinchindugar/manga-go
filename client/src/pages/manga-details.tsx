...
import { Play, Star, Clock, BookOpen, Lock, Coins, Heart, Share2 } from "lucide-react";
          ...
          <div className="flex-1 pt-4">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {manga.genre.map(genre => (
                <Badge key={genre} variant="secondary" className="px-3 py-1 text-sm bg-secondary/50 hover:bg-secondary">
                  {genre}
                </Badge>
              ))}
            </div>
            ...
            <div className="bg-card rounded-xl border border-white/5 overflow-hidden">
              <div className="p-4 border-b border-white/5 flex items-center justify-between bg-muted/20">
                ...
                <span className="text-xs text-muted-foreground uppercase tracking-widest font-semibold">Latest Update: 2 Days Ago</span>
              </div>
              <ScrollArea className="h-[500px]">
                <div className="divide-y divide-white/5">
                  {MOCK_CHAPTERS.map((chapter) => (
                    <Link key={chapter.id} href={chapter.isLocked ? "#" : `/read/${manga.id}/${chapter.id}`}>
                      <a className={`flex items-center justify-between p-4 hover:bg-white/5 transition-colors group ${chapter.isLocked ? 'opacity-70' : ''}`}>
                        <div className="flex flex-col gap-1">
                          <span className="font-semibold group-hover:text-primary transition-colors text-base">
                            {chapter.title}
                          </span>
                          <span className="text-xs text-muted-foreground">20 pages • Oct 12, 2024</span>
                        </div>
